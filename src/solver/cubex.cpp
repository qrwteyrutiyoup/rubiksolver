...
bool Cubex::operator==(const Cubex &q)
{
...
bool Cubex::operator!=(const Cubex &a) { return !(operator==(a)); }
...
bool Cubex::XML(int a, bool n = true)
{
...
bool Cubex::XMR(int a, bool n = true)
{
  if (a < 1 || a > N) return false;
  Ctemp();
  for (int i = 1; i <= N; i++) {
...
bool Cubex::XMU(int a, bool n = true)
{
  if (a < 1 || a > N) return false;
  Ctemp();
  for (int i = 1; i <= N; i++) {
...
bool Cubex::XMD(int a, bool n = true)
{
  if (a < 1 || a > N) return false;
  Ctemp();
  for (int i = 1; i <= N; i++) {
...
bool Cubex::XMC(int a, bool n = true)
{
  if (a < 1 || a > N) return false;
  Ctemp();
  for (int i = 1; i <= N; i++) {
...
bool Cubex::XMA(int a, bool n = true)
{
  if (a < 1 || a > N) return false;
  Ctemp();
  for (int i = 1; i <= N; i++) {
...
void Cubex::UL() { XML(N); } // rotate top left
void Cubex::UR() { XMR(N); } // rotate top right
void Cubex::DL() { XML(1); } // rotate bottom left
void Cubex::DR() { XMR(1); } // rotate bottom right
void Cubex::LU() { XMU(1); } // rotate left up
void Cubex::LD() { XMD(1); } // rotate left down
void Cubex::RU() { XMU(N); } // rotate right up
void Cubex::RD() { XMD(N); } // rotate right down
void Cubex::FC() { XMC(1); } // rotate front clockwise
void Cubex::FA() { XMA(1); } // rotate front anticlockwise
void Cubex::BC() { XMC(N); } // rotate back clockwise (looking from front)
void Cubex::BA() { XMA(N); } // rotate back anticlockwise (looking from front)
void Cubex::ML() { XML(2); } // rotate middle left
void Cubex::MR() { XMR(2); } // rotate middle right
void Cubex::MU() { XMU(2); } // rotate middle up
void Cubex::MD() { XMD(2); } // rotate middle down
void Cubex::MC() { XMC(2); } // rotate middle clockwise
void Cubex::MA() { XMA(2); } // rotate middle anticlockwise
void Cubex::CL() { for (int i = 1; i <= N; i++) XML(i); } // rotate whole cube left
void Cubex::CR() { for (int i = 1; i <= N; i++) XMR(i); } // rotate whole cube right
void Cubex::CU() { for (int i = 1; i <= N; i++) XMU(i); } // rotate whole cube up
void Cubex::CD() { for (int i = 1; i <= N; i++) XMD(i); } // rotate whole cube down
void Cubex::CC() { for (int i = 1; i <= N; i++) XMC(i); } // rotate whole cube clockwise
void Cubex::CA() { for (int i = 1; i <= N; i++) XMA(i); } // rotate whole cube anticlockwise
void Cubex::XCL() { for (int i = 1; i <= N; i++) XML(i, false); } // rotate whole cube left w/o altering centers
void Cubex::XCR() { for (int i = 1; i <= N; i++) XMR(i, false); } // rotate whole cube right w/o altering centers
void Cubex::XCU() { for (int i = 1; i <= N; i++) XMU(i, false); } // rotate whole cube up w/o altering centers
void Cubex::XCD() { for (int i = 1; i <= N; i++) XMD(i, false); } // rotate whole cube down w/o altering centers
void Cubex::XCC() { for (int i = 1; i <= N; i++) XMC(i, false); } // rotate whole cube clockwise w/o altering centers
void Cubex::XCA() { for (int i = 1; i <= N; i++) XMA(i, false); } // rotate whole cube anticlockwise w/o altering centers
// scramble up the cube model
void Cubex::ScrambleCube()
{
...
int Cubex::FindEdge(int a, int b)
{
  int f = 0, x, y, z; fx = 0; fy = 0; fz = 0;
  for (int i = -1; i <= 1; i = i + 2) {
...
int Cubex::FindCorn(int a, int b, int c)
{
  int f = 0, x, y, z; fx = 0; fy = 0; fz = 0;
  for (int i = -1; i <= 1; i = i + 2) {
    for (int j = -1; j <= 1; j = j + 2) {
...
const string Cubex::Concise(string a)
{
  ...
  while (c <= (int)s.length() / 3) {
    ...
    if (s1 == "C") {
      ...
      t = "";
      for (int i = c + 1; i <= (int)s.length() / 3; i++) {
        t1 = s.substr(i * 3 - 3, 1);
        t2 = s.substr(i * 3 - 2, 1);
        if      (t1 == "U") { t += zz; }
        else if (t1 == "D") { t += yy; }
  ...
  while (g > 0) {
    ...
    b = 0;
    while (b <= (int)s.length() / 3 - 1 && (int)s.length() / 3 > 0) {
      s1 = s.substr(b * 3, 2);
      t1 = s1.substr(0, 1);
      if      (t1 == "U") { t3 = "D"; }
      else if (t1 == "D") { t3 = "U"; }
      ...
      s2 = s.substr(b * 3 + 3, 2);
      t2 = s2.substr(0, 1);
      while ((t2 == t1 || t2 == t3) && c <= (int)s.length() / 3 - b - 2 && s != "") {
        if (t2 == t1 && c > 0) {
          t = s.substr(0, b * 3 + 3);
          t += s.substr(b * 3 + c * 3 + 3, 3);
          t += s.substr(b * 3 + 3, c * 3);
          t += s.substr(b * 3 + c * 3 + 6, (int)s.length() - (b * 3 + c * 3 + 6));
          s = t;
          c = s.length() / 3;
        }
        else if (t2 == t3) {
    ...
    b = 0;
    while (b <= (int)s.length() / 3 - 2 && (int)s.length() / 3 >= 3) {
      s1 = s.substr(b * 3, 2);
      s2 = s.substr(b * 3 + 3, 2);
      s3 = s.substr(b * 3 + 6, 2);
      t1 = s1.substr(0, 1);
    ...
    b = 0;
    while (b <= (int)s.length() / 3 - 2 && s.length() / 3 >= 2) {
      t1 = s.substr(b * 3, 1);
      t2 = s.substr(b * 3 + 3, 1);
      s1 = s.substr(b * 3 + 1, 1);
      s2 = s.substr(b * 3 + 4, 1);
...
const string Cubex::TopCorners()
{
  string s = "";
  if (!cubeinit) return s;
  string a = ""; int b = 0, c, c1, f, f1, m = 0;
  while (!(FindCorn(1, 2, 5) == 2 && FindCorn(2, 1, 5) == -3 &&
   ...
   FindCorn(1, 5, 4) == 2 && FindCorn(5, 1, 4) == 1)) {
    ...
    f = FindCorn(1, c, c1); f1 = FindCorn(c, 1, c1);
    switch (f) {
      case 2:
        switch (f1) {
          case 3:
